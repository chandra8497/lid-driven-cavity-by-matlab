...
figure(2)
    plot(u_final(:,N/2),1-y(:));
    ylim([0 1])
    title(sprintf('Plot of u at y=0.5 centerline for %sX%s',num2str(N),num2str(N)));
    xlabel('u');
    ylabel('y');
    grid on;
...
figure(3);
    ...
    xlim([0 1])
    grid on;
    title(sprintf('Plot of v at y=0.5 centerline for %sX%s',num2str(N),num2str(N)));
    ylabel('v');
    xlabel('x');
